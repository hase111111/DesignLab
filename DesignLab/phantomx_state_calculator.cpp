...
bool PhantomXStateCalclator::IsVaildJointState(const RobotStateNode& node, const std::array<HexapodJointState, HexapodConst::kLegNum>& joint_state) const
{
	...
	for (int i = 0; i < HexapodConst::kLegNum; i++)
	{
		if (! IsVaildJointStateOneLeg(i, node, joint_state[i])) 
		{
			res = false;
		}
	}
...
bool PhantomXStateCalclator::IsVaildJointStateOneLeg(const int leg_index, const RobotStateNode& node, const HexapodJointState& joint_state) const
{
	assert(joint_state.local_joint_position.size() == 4);
	...
	if (joint_state.local_joint_position[3] != node.leg_pos[leg_index]) { return false; }
