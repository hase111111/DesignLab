...
{
    using leg_func::IsGrounded;
    using dxlib_util::ConvertToDxlibVec;
    using dxlib_util::DrawHexagonalPrism;
    ...
    DrawHexagonalPrism(vertex, PhantomXMkIIConst::kBodyHeight, kColorBody);

    // 重心の描画．
    DrawSphere3D(ConvertToDxlibVec(draw_node_.center_of_mass_global_coord),
        kJointRadius * 1.5f, kSphereDivNum, kColorJoint, kColorJoint, TRUE);

    // 脚を描画する．
    for (int i = 0; i < HexapodConst::kLegNum; i++)
    {
        // 脚の色を遊脚・接地で変更する．
        const unsigned int kLegBaseColor = IsGrounded(draw_node_.leg_state, i) ?
            kColorLeg : kColorLiftedLeg;
        const unsigned int kJointColor = IsGrounded(draw_node_.leg_state, i) ?
            kColorJoint : kColorLiftedJoint;
        ...
        for (int j = 0; j < 3; j++)
        {
            const VECTOR start = ConvertToDxlibVec(
              converter_ptr_->ConvertLegToGlobalCoordinate(
            ...
            const VECTOR end = ConvertToDxlibVec(
              converter_ptr_->ConvertLegToGlobalCoordinate(
            ...
            {
                DrawCapsule3D(start, end, kLegRadius, kCapsuleDivNum,
                    kLegBaseColor, kLegBaseColor, TRUE);
            }
            else
            {
                DrawCapsule3D(start, end, kLegRadius, kCapsuleDivNum,
                    kColorErrorJoint, kColorErrorJoint, TRUE);
            }
        }
        ...
        DrawSphere3D(
          ConvertToDxlibVec(converter_ptr_->ConvertLegToGlobalCoordinate(
            draw_node_.leg_pos[i], i,
            draw_node_.center_of_mass_global_coord,
            draw_node_.posture, true)),
          kJointRadius / 2,
          kSphereDivNum,
          kJointColor,
          kJointColor,
        ...
        DrawSphere3D(
          ConvertToDxlibVec(converter_ptr_->ConvertLegToGlobalCoordinate(
            draw_node_.leg_reference_pos[i], i,
            draw_node_.center_of_mass_global_coord,
            draw_node_.posture, true)),
          kJointRadius / 3,
          kSphereDivNum,
